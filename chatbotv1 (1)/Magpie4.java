...
// }};
  ...
  Boolean repeat = false;
    ...
    */
    //
    if (findKeyword(statement, "hate") >= 0 
        || findKeyword(statement, "despise") >= 0
        || findKeyword(statement, "evil") >= 0
        || statement.contains("astard"))
    {
      State = 3;
    }
    //goofy
        ...
        || statement.contains("stupid")
        || statement.contains("idiot")
        || statement.contains("not smart")
        || statement.contains("goofy ahh"))
    {
      State = 7;
    }
      ...
      State = 4;
        ...
        || findKeyword(statement, "rapscallion") >= 0
        || findKeyword(statement, "rambunctious") >= 0
        || findKeyword(statement, "zany") >= 0
        || statement.contains("flabbergast")
        || statement.contains("mollywhop")
        || statement.contains("defenestr")
        || statement.contains("befuddl")
...
monkeFacts.add("white-faced capuchin monke say hello by sticking finger up other monke nose");
    ...
    {
			response = "\"never make hate monke only makes love\" - sun 'monke' tzu, The Art of Monke";
		}
    
    else if (State == 1)
      {
        response = "WOOO OOO AAA AAAHH MONKE HAPPY FOR U";
      }
      else if ((State == 1) && negative)
      {
    ...
    {
			response = "would you like to hear about monkes?";
		}
